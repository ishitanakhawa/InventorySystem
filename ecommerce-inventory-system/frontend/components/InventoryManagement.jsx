...
const InventoryManagement = ({ user }) => {
  ...
  const loadData = async (silent = false) => {
    ...
    setError("");
    try {
      const [whData, alertData, products] = await Promise.all([
      ...
      if (whData.length > 0 && !whData.find((w) => w.id === selectedWarehouse)) {
  ...
  const syncStock = async () => {
    setSyncing(true);
    try {
      await Promise.all([api.get("/api/warehouses"), api.get("/api/products")]);
      await loadData(true);
      setSuccess("Warehouse stock synchronized with catalog in real time.");
    } catch {
      setError("Sync failed — ensure the C++ backend is running on port 8080.");
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleRouteQuery = async (targetId, algo = selectedAlgo) => {
    ...
    try {
      const response = await fetch(
        `/api/warehouses/shortest-path?from=${selectedWarehouse}&to=${targetId}&algo=${algo}`
      );
  ...
  const handleStockTransfer = async (fromWarehouseId, toWarehouseId, productId, quantity) => {
    ...
    try {
      const response = await fetch("/api/warehouses/transfer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          fromWarehouseId,
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* Smart alerts + real-time stock strip */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            <Bell className="w-5 h-5 text-warning" />
            <h3 className="font-bold">Smart Alerts</h3>
            <span className="chip chip-gray text-xs">{alerts.length} active</span>
          </div>
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {alerts.length === 0 ? (
              <p className="text-sm text-gray-500">No alerts — inventory levels normal.</p>
            ) : (
              alerts.map((a) => (
                <div
                  key={`${a.type}-${a.id}`}
                  className={`p-3 rounded-12 text-sm border ${
                    a.severity === "CRITICAL"
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold">Real-Time Stock</h3>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto text-sm">
            {liveProducts.slice(0, 6).map((p) => (
              <div key={p.id} className="flex justify-between py-1 border-b border-border/50">
                <span>{p.name}</span>
                <span className={p.stock > p.maxStock * 0.85 ? "text-warning font-bold" : ""}>
                  {p.stock} / {p.maxStock}
                </span>
              </div>
            ))}
