...
const ProductCatalog = ({ user }) => {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      console.warn("Failed to load products from backend. Using offline mock catalog.");
      // Fallback to robust mock data with basePrice and price for dynamic pricing demo
      const mockProducts = [
        { id: 1, name: "Laptop", category: "Electronics", basePrice: 999.99, price: 1099.99, popularity: 85, stock: 50, maxStock: 150, rating: 4.8, turnoverRate: 0.65 },
        { id: 2, name: "Mouse", category: "Electronics", basePrice: 29.99, price: 32.99, popularity: 90, stock: 200, maxStock: 300, rating: 4.3, turnoverRate: 0.85 },
        { id: 3, name: "Keyboard", category: "Electronics", basePrice: 79.99, price: 87.99, popularity: 88, stock: 150, maxStock: 250, rating: 4.5, turnoverRate: 0.72 },
        { id: 4, name: "Monitor", category: "Electronics", basePrice: 349.99, price: 349.99, popularity: 75, stock: 80, maxStock: 200, rating: 4.6, turnoverRate: 0.55 },
        { id: 5, name: "Headphones", category: "Electronics", basePrice: 149.99, price: 164.99, popularity: 82, stock: 120, maxStock: 200, rating: 4.2, turnoverRate: 0.60 },
        { id: 6, name: "T-Shirt", category: "Clothing", basePrice: 19.99, price: 11.99, popularity: 95, stock: 300, maxStock: 500, rating: 4.7, turnoverRate: 0.95 },
        { id: 7, name: "Jeans", category: "Clothing", basePrice: 59.99, price: 65.99, popularity: 80, stock: 180, maxStock: 250, rating: 4.1, turnoverRate: 0.70 },
        { id: 8, name: "Jacket", category: "Clothing", basePrice: 89.99, price: 76.49, popularity: 50, stock: 100, maxStock: 150, rating: 4.0, turnoverRate: 0.45 },
        { id: 9, name: "Sneakers", category: "Footwear", basePrice: 129.99, price: 142.99, popularity: 88, stock: 150, maxStock: 200, rating: 4.9, turnoverRate: 0.80 },
        { id: 10, name: "Boots", category: "Footwear", basePrice: 159.99, price: 135.99, popularity: 45, stock: 90, maxStock: 120, rating: 4.4, turnoverRate: 0.40 },
      ];
  ...
  const handleDeleteProduct = async (id) => {
    ...
    if (!confirm("Are you sure you want to delete this product? This will synchronize across all warehouses.")) return;
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Update listing specs, track ratings, and monitor demand-based pricing adjustments
            </p>
          </div>
        </div>
          ...
          <div className="flex bg-gray-100 rounded-8 p-1">
            <button
              ...
              className={`p-2 rounded-6 transition-colors ${
                viewMode === "grid" ? "bg-white shadow-1 text-primary" : "text-gray-500"
              }`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-2 rounded-6 transition-colors ${
                viewMode === "list" ? "bg-white shadow-1 text-primary" : "text-gray-500"
              }`}
              title="List View"
            >
      ...
      {error && (
        <div className="alert alert-warning animate-slideUp">
          ...
          <button onClick={() => setError("")} className="ml-auto font-bold text-xs uppercase">Dismiss</button>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-medium text-gray-600">
          <div className="bg-white/80 p-3 rounded-12 border border-gray-100 space-y-1">
            ...
            <p className="text-gray-500 text-[11px]">
              Catalog items with popularity score &gt; 80 automatically undergo a +10% price markup. Items under 60 undergo -15% markdown for overstock clearance.
            </p>
          </div>
          <div className="bg-white/80 p-3 rounded-12 border border-gray-100 space-y-1">
            <p className="font-bold text-gray-900 flex items-center gap-1.5">
            ...
            <p className="text-gray-500 text-[11px]">
              Holiday/Peak shopping window (6 PM – 10 PM) activates a global +5% seasonal markup multiplier calculated in real-time.
            </p>
          </div>
          <div className="bg-white/80 p-3 rounded-12 border border-gray-100 space-y-1">
            <p className="font-bold text-gray-900 flex items-center gap-1.5">
            ...
            <p className="text-gray-500 text-[11px]">
              T-Shirt (ID #6) is designated as active flash sale anchor. Slashed price enjoys an automatic 40% discount off original base catalog specs.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card space-y-4 animate-slideUp">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center pt-4 border-t border-border">
          ...
          {/* Sort */}
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-600 font-bold uppercase tracking-wider">Sort by:</span>
            <select
              value={sortBy}
      ...
      {filteredProducts.length === 0 ? (
        <div className="card text-center py-16 animate-fadeIn">
          ...
          <h3 className="text-md font-bold text-gray-900 mb-2">No matching products found</h3>
          <p className="text-xs text-gray-500 mb-4">Try clearing active category or price bounds filters</p>
          <button onClick={clearFilters} className="btn-primary py-2 text-xs font-bold">
            Clear Filters
          </button>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`card border border-border flex flex-col justify-between group ${
                  viewMode === "list" ? "md:flex-row items-center gap-6 p-4" : "p-5"
                }`}
                style={{ animationDelay: `${index * 30}ms` }}
              >
                <div className={`relative ${viewMode === "list" ? "w-20 h-20 flex-shrink-0" : "w-full mb-4"}`}>
                  <div className={`w-full bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200/55 rounded-12 flex items-center justify-center ${
                ...
                <div className={`flex-grow space-y-3 text-left ${viewMode === "list" ? "grid grid-cols-1 md:grid-cols-4 gap-4 items-center" : ""}`}>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full border uppercase tracking-wider ${m.style}`} title={m.desc}>
                        {m.text}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center text-xs font-semibold text-gray-500">
                      ...
                      <span className={product.stock < 50 ? "text-error font-bold" : "text-emerald-700 font-bold"}>
                        {product.stock} units
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${Math.min((product.stock / (product.maxStock || 250)) * 100, 100)}%` }}
                      ></div>
                    </div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
                      Turnover Velocity: {(product.turnoverRate || 0.5).toFixed(2)}x
                    </p>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card p-6 w-full max-w-md animate-scaleIn">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-900">Add New Product Specs</h3>
      ...
      {editingProduct && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card p-6 w-full max-w-md animate-scaleIn">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-900">Edit Product Specifications</h3>
